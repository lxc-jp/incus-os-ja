# IncusOS
IncusOSは、Incusを安全・確実に動かすことだけを考えて設計されたイミュータブルなOSです。
UEFIセキュアブートやTPMのような、モダンなセキュリティの機能を使って、安全なブート体験とシームレスな完全なディスクの暗号化を提供します。
...
すべてのインストールはデフォルトでは`stable`チャンネルを使用します。このチャンネルでは、Linuxカーネルや重要なセキュリティーの問題の最新の安定版のバグフィクスリリースを含む更新が、通常は週に少なくとも一回あります。
